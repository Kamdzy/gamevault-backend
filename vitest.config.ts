...
export default defineConfig({
  test: {
    ...
    include: ["src/**/*.spec.ts"],
    // Fork: suites that boot a full Nest app (status.e2e) exceed the 5s
    // default under parallel load — it is a bootstrap cost, not a slow
    // assertion. Carried over from the fork's jest.config.ts.
    testTimeout: 30000,
    coverage: {
      provider: "v8",
      include: ["src/**/*.ts"],
